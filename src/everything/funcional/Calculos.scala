...
object Calculos {
  ...
  def semanaPeso(atual:Double, pretendido:Double):Int = (pretendido-atual).toInt.abs

  def poupanca(t:Tracker):Int = (t.registos map (x=> x.meta-x.dado) foldRight 0 )(_+_) match {
    case x if x < 0 => 0
    case y => y
  }
  def ganho(t:Tracker):Int = (t.registos map (x=> x.dado-x.meta) foldRight 0 )(_+_) match {
    case x if x < 0 => 0
    case y => y
  }
